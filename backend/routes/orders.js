...
router.get('/notifications/my', auth(), (req, res) => {
...
// Place order
router.post('/', auth(['retailer']), (req, res) => {
  const { items, notes, delivery_address } = req.body;
  if (!items || !items.length) return res.status(400).json({ message: 'No items' });

  const grouped = {};
  for (const item of items) {
    const product = db.prepare('SELECT * FROM products WHERE id = ? AND is_active = 1').get(parseInt(item.product_id));
  ...
  for (const [wholesaler_id, orderItems] of Object.entries(grouped)) {
    ...
    const ord = db.prepare('INSERT INTO orders (retailer_id, wholesaler_id, total_amount, notes, delivery_address) VALUES (?,?,?,?,?)').run(req.user.id, parseInt(wholesaler_id), total, notes || '', delivery_address || '');
    const orderId = Number(ord.lastInsertRowid);
    for (const i of orderItems) {
      db.prepare('INSERT INTO order_items (order_id, product_id, quantity, unit_price, subtotal) VALUES (?,?,?,?,?)').run(orderId, i.product.id, i.quantity, i.product.price, i.product.price * i.quantity);
    }
    db.prepare("INSERT INTO deliveries (order_id, delivery_address, status) VALUES (?,?,'pending')").run(orderId, delivery_address || '');
    notify(parseInt(wholesaler_id), 'New Order', 'طلب جديد', `New order #${orderId} received`, `تلقيت طلباً جديداً رقم #${orderId}`, 'order');
    orders.push(orderId);
  }
...
// Get orders
router.get('/', auth(['retailer', 'wholesaler', 'admin']), (req, res) => {
...
router.get('/:id', auth(), (req, res) => {
  ...
  `).all(order.id);
  order.delivery = db.prepare('SELECT * FROM deliveries WHERE order_id = ?').get(order.id);
  res.json(order);
});

// Update order status
router.patch('/:id/status', auth(['wholesaler', 'admin']), (req, res) => {
  const { status } = req.body;
  const order = db.prepare('SELECT * FROM orders WHERE id = ?').get(parseInt(req.params.id));
  if (!order) return res.status(404).json({ message: 'Not found' });
  if (req.user.role === 'wholesaler' && order.wholesaler_id !== req.user.id) return res.status(403).json({ message: 'Forbidden' });

  db.prepare("UPDATE orders SET status=?, updated_at=datetime('now') WHERE id=?").run(status, parseInt(req.params.id));
  notify(order.retailer_id, 'Order Updated', 'تحديث الطلب', `Order #${order.id} is now ${status}`, `طلبك رقم #${order.id} أصبح ${status}`, 'order');
  res.json({ message: 'Status updated' });
});

// Cancel order
router.patch('/:id/cancel', auth(['retailer']), (req, res) => {
  const order = db.prepare('SELECT * FROM orders WHERE id = ? AND retailer_id = ?').get(parseInt(req.params.id), req.user.id);
  if (!order) return res.status(404).json({ message: 'Not found' });
  if (!['pending'].includes(order.status)) return res.status(400).json({ message: 'Cannot cancel at this stage' });
  db.prepare("UPDATE orders SET status='cancelled', updated_at=datetime('now') WHERE id=?").run(parseInt(req.params.id));
  notify(order.wholesaler_id, 'Order Cancelled', 'إلغاء طلب', `Order #${order.id} was cancelled`, `تم إلغاء الطلب رقم #${order.id}`, 'warning');
